import { BORDER, FOOTERSECONDARY, ORANGE, screens, WHITE } from "../utils/styling"
import { Container } from "./Container"
import { FLogo } from './Logo';
import { BACKGROUND, Span, FOOTERPRIMARY, BORDERRADIUS } from './../utils/styling';
...
const MainFooter = () => {
  ...
  return <Stack direction={tablet ? "column" : "column"} alignItems="center" justifyContent="space-between" width="100%" padding={"20px"} >
...
const Phone = () => {
  const tablet = useMediaQuery(`(max-width:${screens[1]}px)`)
  return <Stack width={'100%'} justifyContent='center' alignItems={tablet ? "center" : "flex-end"}direction={tablet ? 'row' : 'column'} spacing={2}>
...
const FooterLogo = () => {
  ...
  return <Stack width="100%" maxHeight={100}  alignItems="center" style={{padding: "20px 0px", textAlign: 'center'}} spacing={2} direction={tablet ? "column" : "row"}>
    <Stack direction="row" height='100%' alignItems="center" spacing={2}>
      <FLogo imgStyle={{maxWidth: 323, height: 'auto', width: '100%'}}/>
      {!mobile && 
        <>
          <span style={{ width: 2, height: 50, background: BORDER}}></span>
...
const FooterContact = () => {
  ...
  return <Stack width="100%" alignItems="flex-start" justifyContent={"center"} style={{padding: "20px 0px", textAlign: 'center'}} spacing={1}>
    ...
    <Stack width="100%" style={{ background: BACKGROUND, pasition: 'relative'}} alignItems='center'justifyContent='center'>
      ...
      <Stack width="100%"
        ...
        <Stack style={{zIndex: 10}}>
          ...
          <span style={{width: '100%', background: ORANGE, padding: 3}}></span>
        </Stack>
      </Stack>
    </Stack>
  </Container>
